...
impl FortranPhysics {
    ...
    ///
    /// ИСПРАВЛЕНО (диагностика — жалоба "ФПС скачет" ПОСЛЕ фиксов
    /// стриминга/hot-reload/culling; `[UPDATE-BREAKDOWN]` показал
    /// `physics_ms` в десятки-сотни мс при том, что broad/narrow/сам
    /// солвер — единицы мс): `std::thread::scope` выше СОЗДАЁТ РЕАЛЬНЫЕ
    /// OS-потоки — и делал это КАЖДЫЙ КАДР (60 раз в секунду), даже когда
    /// тел мало (в демо-сцене — 123, из них активны всего 6, см.
    /// `[PHYS-STATS] active=6`). Создание/уничтожение потоков — системный
    /// вызов ОС, стоимость которого на слабом/загруженном железе может
    /// доходить до десятков миллисекунд (диспетчер потоков Windows,
    /// парковка ядер энергосбережением) — совершенно неоправданно ради
    /// работы, которая на одном потоке для сотни простых struct'ов
    /// занимает микросекунды. Порог `PARALLEL_INTEGRATE_THRESHOLD` ниже
    /// — при малом числе тел (типичный случай, особенно на "минимум
    /// 10-летнее железо" из ТЗ) НИ ОДНОГО потока не создаётся вообще:
    /// прямой однопоточный вызов Fortran на весь срез сразу. Параллелизм
    /// включается только когда тел действительно много — там, где он
    /// реально окупает свою стоимость создания потоков.
    pub fn batch_integrate(&mut self, dt: f32, gravity: f32, num_threads: usize) {
        if self.bodies.is_empty() {
            return;
        }

        const PARALLEL_INTEGRATE_THRESHOLD: usize = 512;
        if self.bodies.len() < PARALLEL_INTEGRATE_THRESHOLD {
            let n = self.bodies.len() as i32;
            unsafe {
                batch_integrate(self.bodies.as_mut_ptr(), n, dt, gravity, 1, n);
            }
            return;
        }
