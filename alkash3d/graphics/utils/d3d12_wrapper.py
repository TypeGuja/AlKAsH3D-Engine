...
if not _lib_path.is_file():
    ...
    _lib_path = Path(__file__).parent.parent.parent / f"alkash3d_dx12{_ext}"
    if not _lib_path.is_file():
...
# ИСПРАВЛЕНО: убрал параметр upload, теперь 5 параметров
...
_create_constant_buffer_view = _load_func(
    ...
    [
        ...
        ctypes.c_void_p,  # CPU‑descriptor‑handle
    ],
    required=False,
)
...
def force_cleanup():
    """Принудительная очистка всех ресурсов DirectX 12."""
    if _force_cleanup:
        _force_cleanup()
...
def update_subresource(buffer: Any, data: bytes, size: int = None) -> None:
    ...
    _update_subresource(_to_cvoid(buffer), ctypes.c_void_p(ctypes.addressof(raw)), ctypes.c_size_t(size))
...
def update_texture(texture: ctypes.c_void_p, data: bytes, width: int, height: int) -> None:
    ...
    ptr_val = texture.value if hasattr(texture, 'value') else int(texture) if texture else 0
    stub_pointers = [0xDEADBEEF, 0xDEADF00D, 0xFEEDC0DE, 0x12345678, 0x87654321]

    if ptr_val in stub_pointers:
        debug_print(f"update_texture: skipping stub pointer {ptr_val:#x}")
        return
...
def create_descriptor_heap(device: ctypes.c_void_p, num_descriptors: int, heap_type: int,
                           shader_visible: bool = False) -> ctypes.c_void_p:
...
def release_resource(resource: Any) -> None:
    ...
    # Проверяем на stub-указатели
    ptr_val = resource.value if hasattr(resource, 'value') else int(resource) if resource else 0
    stub_pointers = [0xDEADBEEF, 0xDEADF00D, 0xFEEDC0DE, 0x12345678, 0x87654321]

    if ptr_val in stub_pointers:
        debug_print(f"release_resource: skipping stub pointer {ptr_val:#x}")
        return

    # Проверяем на слишком маленькие адреса (уже освобождены)
    if ptr_val < 0x10000 and ptr_val > 0:
        debug_print(f"release_resource: suspiciously small pointer {ptr_val:#x}, skipping")
        return
