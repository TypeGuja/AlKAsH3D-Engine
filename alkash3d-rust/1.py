...
def main():
    ...
    # 6. Создаём дескрипторную кучу (для WARP отключаем shader_visible)
    print("\n6. Creating descriptor heap...")
    shader_visible = not is_warp
    heap = lib.create_descriptor_heap(device, 10, 2, shader_visible)
    ...
    if heap == 0:
        ...
        if shader_visible:
            ...
            print("   ℹ️  CPU-visible heap (no GPU handle)")

    # 7. Создаём буфер
    print("\n7. Creating vertex buffer...")
    vertex_data = b'\x00\x00\x00\x00' * 36
    ...
    # Не вызываем force_cleanup - он вызывает access violation
    # lib.force_cleanup()
