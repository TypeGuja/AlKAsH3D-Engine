...
impl CarPhysicsState {
    ...
    pub fn step(
        ...
        for (i, local_pos) in wheel_local_positions.iter().enumerate() {
            ...
            // ИСПРАВЛЕНО (полноценная физика — запрос луча против сцены,
            // см. `AlkashEngine::physics_raycast`/`kernels/raycast.f90` в
            // alkash3d-inertial): раньше здесь БЫЛ жёстко зашитый плоский
            // `ground_y` — земля физически не существовала как тело,
            // только как визуальный меш (см. `main_car.rs::setup_ground`),
            // так что рампы/ямы/другие физические тела под колесом были
            // невидимы для подвески. Теперь честный raycast СТРОГО ВНИЗ в
            // мировых координатах (см. прежнее обоснование "не вдоль
            // локальной оси стойки подвески" — оно остаётся в силе, это
            // сознательное упрощение, отдельное от источника земли) против
            // РЕАЛЬНОЙ физической геометрии — `body_id` (собственный кузов)
            // исключён из результата (иначе точка крепления колеса, лежащая
            // на границе/внутри его же box-коллайдера, почти всегда
            // "находила бы" саму машину раньше земли). Если raycast ничего
            // не нашёл (например физическая плоскость пола ещё не
            // добавлена сценой через `add_plane` — визуальный меш пола сам
            // по себе не физическое тело) — откатываемся на старое плоское
            // приближение по `ground_y`, чтобы подвеска не "проваливалась"
            // молча там, где физической земли пока попросту нет.
            let max_reach = params.suspension_rest_length + params.suspension_max_travel;
            let ground_dist = engine
                .physics_raycast(world_attach.to_array(), [0.0, -1.0, 0.0], max_reach, Some(body_id))
                .map(|hit| hit.distance)
                .unwrap_or_else(|| world_attach.y - ground_y);
