...
pub struct MaterialDefinition {
    ...
    // `NO_TEXTURE` = слот не используется. Эдитор пока не поддерживает
    // текстурные материалы (см. `material::Material` в alkash3d-editorapp —
    // там только цвет/metallic/roughness/emissive, без путей к текстурам),
    // так что при экспорте ИЗ эдитора эти поля сегодня всегда `NO_TEXTURE` —
    // поля зарезервированы заранее, чтобы добавление текстурных материалов
    // в эдитор в будущем не потребовало версионирования этого формата.
