...
/// ДОБАВЛЕНО (максимальная графика — MSAA): число сэмплов на пиксель для
/// основного цветового/depth-таргета (`Renderer::hdr_target`/
/// `depth_stencil`, см. `RenderTexture::create_hdr_target`/
/// `create_depth_stencil` в render.rs). 4x — стандартный практический
/// компромисс (та же логика выбора, что у `NUM_CASCADES` выше: заметно
/// лучше 2x по качеству сглаживания краёв геометрии, заметно дешевле 8x
/// по видеопамяти/полосе пропускания на зафиксированном минимуме железа).
/// Основной цветовой проход рисует В MSAA-таргет, затем разрешается
/// (`ResolveSubresource`, см. `render_frame`) в одноимпловый
/// `Renderer::hdr_resolved` ДО того, как bloom/volumetric/tonemap его
/// читают — те проходы ничего не знают про MSAA вообще, читают уже
/// разрешённую текстуру, как и раньше. depth-таргет ОСТАЁТСЯ
/// многосэмпловым (не разрешается отдельно) — volumetric raymarch читает
