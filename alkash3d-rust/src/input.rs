...
pub mod keys {
    ...
    /// ДОБАВЛЕНО (My Summer Car-like демо — переключение камеры вида от
    /// третьего лица при вождении/пешком): виртуальный код клавиши C.
    pub const C: u32 = 0x43;
    /// ДОБАВЛЕНО (My Summer Car-like демо — ручной тормоз машины):
    /// виртуальный код клавиши R (Space уже занят под "прыжок"/тормоз
    /// пешком в других демо и под резкое торможение здесь — R выбран,
    /// чтобы не конфликтовать с уже устоявшимся SPACE=выход в других
    /// сценах движка).
