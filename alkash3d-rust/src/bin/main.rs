...
fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    // ИСПРАВЛЕНО (баг: "доходя до определённого участка весь мир просто
    // пропадает, хотя по идее он там ещё должен быть"): `Camera::new`
    // ставит `far = 100.0` (см. camera.rs), и этот файл его никогда не
    // переопределял. `setup_world_streaming` ниже грузит чанки
    // `demo_world`/пользовательского `.alworld` вплоть до
    // `StreamingConfig::unload_distance` (250.0 по умолчанию, см.
    // `AlworldFile::new` в alworld_format.rs) — то есть контент честно
    // стримится в `Scene` и должен быть виден, но perspective-проекция
    // (`perspective()` в math.rs) отсекала всё дальше 100 юнитов от камеры
    // ДО того, как до дела доходил culling — поэтому мир не тускнел
    // постепенно, а обрывался резко, ровно на границе far-плоскости, стоило
    // отойти от точки спавна дальше чем на 100м. Тот же класс бага уже был
    // однажды исправлен в main_test.rs (см. `CAMERA_FAR_PLANE` там) —
    // здесь то же самое, с запасом сверх `unload_distance`, чтобы
    // загруженные, но ещё не выгруженные чанки не срезались по границе.
    engine.camera.far = 300.0;
...
fn setup_lights(engine: &mut AlkashEngine) {
    ...
    // раньше пропадала карта" — та же причина, что и обрыв мира выше:
    // сетка каллинга FirstFires — фиксированный куб [-far_plane,
    // far_plane]^3 вокруг МИРОВОГО НАЧАЛА КООРДИНАТ (не камеры, см.
    // `LightState::new` в alkash3d-FirstFires/src/lib.rs), заведённый ОДИН
    // раз при `init_lights()`. far_plane=200.0 соответствовал старому
    // `camera.far`, который только что поднят до 300.0 выше — геометрия
    // между 200 и 300 теперь честно рендерится (не отсекается far-
    // плоскостью), но сетка каллинга по-прежнему обрывается на 200:
    // шейдер (`pipeline_main.rs`) при выходе мировой позиции пикселя за
    // `gridDimensions` не добавляет ни одного источника, поэтому свет
    // резко обрывается ровно на границе куба сетки, а не плавно тускнеет.
    // Фикс — тот же приём, что и раньше при подъёме far_plane со 100 до
    // 200 (см. комментарий выше про просадку FPS): far_plane поднят вместе
    // с grid_cell_size (20.0 -> 30.0), чтобы сетка осталась теми же
    // 20x20x20=8000 ячейками (та же память), но накрывала куб
    // [-300,300]^3. lod_distances[2] поднят синхронно по той же причине.
    let config = LightConfig {
        max_lights: 64,
        tile_size: 16,
        far_plane: 300.0,
        lod_distances: [30.0, 60.0, 300.0],
        grid_cell_size: 30.0,
    };
...
fn setup_scene(engine: &mut AlkashEngine) {
    ...
    // ДОБАВЛЕНО (максимальная графика — LOD для мешей): 945 плиток
    // (GRID_HALF_X=52 × GRID_HALF_Z=4) — ровно тот "земля/гараж, много
    // повторяющихся статичных объектов" случай, для которого LOD и
    // задумывался (см. комментарий у `LodGroup` в engine/mod.rs). LOD1 —
    // плоская "верхняя грань" того же размера (см. `Mesh::tile`) — снизу и
    // сбоку у пола почти никогда не видно, а полный куб (12 треугольников)
    // на этом фоне — чистая переплата. Порог 15м — плитки дальше по узкой
    // (8м) улице реально видно "издалека" почти сразу же при ходьбе.
    let tile_mesh_lod1 = engine.add_tile(0.95, 0.75, 0.75, 0.75, 1.0);
    engine.add_lod_group(vec![tile_mesh, tile_mesh_lod1], vec![15.0, f32::MAX]);
