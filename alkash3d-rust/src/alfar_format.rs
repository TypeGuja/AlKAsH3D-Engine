...
/// ДОБАВЛЕНО (по просьбе — "Type: ... ID: ..." для РАЗНЫХ категорий
/// источников света в одной сцене, не только уличных фонарей): в отличие
/// от `LightType` выше (это ГЕОМЕТРИЯ света — точечный/прожектор/
/// направленный/area, как GPU считает освещение), `LightSourceCategory`
/// — это то, ЧЕМ является источник по смыслу сцены: уличный фонарь, фара
/// машины, окно квартиры и т.д. Один объект имеет ОБА поля одновременно
/// (например уличный фонарь — Category::StreetLight + LightType::Point).
///
/// Назначение цвета переехало из места создания конкретной сцены
/// (`create_night_city`) в отдельную переиспользуемую функцию
/// `light_color_for(category, id)` ниже — так фары машин (см.
/// `alcar_format.rs::CarLight`, сейчас `CarLight::default()`) и будущий
/// свет окон квартир смогут воспользоваться той же палитрой/логикой по
/// ID, вместо копирования кода чередования цветов.
#[repr(u32)]
#[derive(Debug, Clone, Copy, PartialEq, Eq)]
pub enum LightSourceCategory {
    StreetLight = 0,
    CarHeadlight = 1,
    CarTaillight = 2,
    CarBlinker = 3,
...
/// Палитра "тип освещения → цвет" для уличных фонарей: тёплый белый
/// (современный LED), натриевый оранжевый (старые лампы), холодный
/// синевато-белый (ртутные/ксеноновые) — фонари разных лет вперемешку,
/// как в реальном городе.
const STREET_LIGHT_COLORS: [[f32; 3]; 3] = [
    [1.0, 0.95, 0.85], // тёплый белый (LED)
    [1.0, 0.65, 0.25], // натриевый оранжевый (старые лампы)
    [0.75, 0.85, 1.0], // холодный синевато-белый (ртуть/ксенон)
];

/// Палитра для света в окнах квартир (задел на будущее — самих окон как
/// источников света в сцене пока нет, см. комментарий у enum выше):
/// тёплый жёлтый (лампа накаливания/торшер), холодный белый (LED/офисный
/// свет), голубоватый мерцающий (экран телевизора).
const WINDOW_LIGHT_COLORS: [[f32; 3]; 3] = [
    [1.0, 0.85, 0.55], // тёплый жёлтый (лампа накаливания)
    [0.95, 0.97, 1.0], // холодный белый (LED)
    [0.6, 0.75, 1.0],  // голубоватый мерцающий (экран ТВ)
];

/// Возвращает детерминированный цвет для источника света по его категории
/// и ID — единая точка, откуда берётся цвет ЛЮБОГО источника в сцене
/// (уличный фонарь, фара, окно), вместо того чтобы каждое место создания
/// решало это по-своему. Детерминированность по ID (не по времени
/// создания/случайности) гарантирует, что конкретный фонарь/окно с одним
/// и тем же ID всегда получает один и тот же цвет между запусками.
///
/// Категории без своей палитры (фары/поворотники машин) пока возвращают
/// физически обычный цвет соответствующего типа освещения — сама палитра
/// по ID для них появится вместе с реальной генерацией машин в сцене.
pub fn light_color_for(category: LightSourceCategory, id: u32) -> [f32; 3] {
    match category {
        LightSourceCategory::StreetLight => {
            STREET_LIGHT_COLORS[id as usize % STREET_LIGHT_COLORS.len()]
        }
        LightSourceCategory::WindowLight => {
            WINDOW_LIGHT_COLORS[id as usize % WINDOW_LIGHT_COLORS.len()]
        }
        // Фары — обычный белый ксенон/LED (стандартная фара автомобиля).
        LightSourceCategory::CarHeadlight => [0.95, 0.97, 1.0],
        // Задние огни — классический красный.
        LightSourceCategory::CarTaillight => [1.0, 0.15, 0.1],
        // Поворотники — классический оранжевый.
        LightSourceCategory::CarBlinker => [1.0, 0.55, 0.0],
    }
}
...
impl AlfarFile {
    ...
    pub fn create_night_city() -> Self {
        ...
        // ИЗМЕНЕНО (по просьбе — цвет должен зависеть от Type+ID источника,
        // а не быть одной константой на все 20 фонарей): цвет теперь
        // берётся через общую `light_color_for(category, id)` (см. выше в
        // этом файле) — ту же функцию, которой позже воспользуются фары
        // машин (`LightSourceCategory::CarHeadlight`/...) и свет окон
        // квартир (`WindowLight`), когда они появятся в сцене. `id` здесь
        // — тот же индекс `i`, что `add_light` присвоит фонарю как
        // реальный `IndividualLight::id` (см. `id = self.lights.len()`
        // там) — то есть цвет действительно привязан к финальному ID
        // фонаря, а не к произвольному порядку цикла.
        for i in 0..20 {
            ...
            let light = IndividualLight {
                ...
                up: [0.0, 1.0, 0.0],
                color: light_color_for(LightSourceCategory::StreetLight, i as u32),
                intensity: 2.0,
