//! Конвертер FBX -> OBJ с децимацией
...
pub fn convert(fbx_path: &str, output_path: &str) -> Result<()> {
    convert_with_quality(fbx_path, output_path, 0.5)
}

pub fn convert_with_quality(fbx_path: &str, output_path: &str, quality: f32) -> Result<()> {
    println!("[FBX] Converting: {} (quality: {:.0}%)", fbx_path, quality * 100.0);
    ...
    println!("[FBX] Using: {}", blender_path.display());
...
# Импортируем FBX
bpy.ops.import_scene.fbx(filepath=r'{0}')
...
"#,
    ...
    let temp_script = std::env::temp_dir().join("fbx_export.py");
    ...
    if !output.status.success() {
        return Err(anyhow!("FBX export failed"));
    }
