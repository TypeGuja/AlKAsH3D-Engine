// src/app.rs - ПОЛНОСТЬЮ ИСПРАВЛЕННЫЙ
use eframe::egui;
use egui::*;
...
use crate::editor::{Gizmo, EditorCommand, CommandHistory, EditorTool};
use crate::systems::*;
...
impl EditorApp {
    pub fn new(cc: &eframe::CreationContext<'_>) -> Self {
        ...
        let mut systems = Self {
            ...
            camera_position: Vec3::new(5.0, 5.0, 10.0), camera_target: Vec3::ZERO, camera_up: Vec3::UP, camera_fov: 60.0, use_gpu: false,
    ...
    pub fn orbit_camera(&mut self, delta_x: f32, delta_y: f32) {
        ...
        self.camera_position = self.camera_target + Vec3::new(v_angle.cos() * h_angle.cos(), v_angle.sin(), v_angle.cos() * h_angle.sin()) * radius;
    }
    ...
    fn handle_viewport_input(&mut self, ui: &mut Ui, rect: Rect) {
        ...
        if right { if let (Some(cur), Some(last)) = (mouse_pos, self.last_mouse_pos) { self.orbit_camera(cur.x - last.x, cur.y - last.y); } }
        if middle { if let (Some(cur), Some(last)) = (mouse_pos, self.last_mouse_pos) { self.pan_camera(cur.x - last.x, cur.y - last.y); } }
        ui.input(|i| { if i.smooth_scroll_delta.y != 0.0 { self.zoom_camera(i.smooth_scroll_delta.y); } });
        if left && !self.left_mouse_pressed { if !shift { self.scene.selected_ids.clear(); } if let Some(_) = mouse_pos { let mut closest_id = None; let mut min_dist = f32::MAX; for (&id, _) in &self.scene.objects { let p = self.scene.get_world_transform(id).position; let d = (p - self.camera_position).length(); if d < min_dist { min_dist = d; closest_id = Some(id); } } if let Some(id) = closest_id { self.scene.select(id, true); } } }
        self.left_mouse_pressed = left;
        self.last_mouse_pos = mouse_pos;
        if let Some(obj) = self.scene.selected_objects().first() { self.gizmo.update(obj.transform); self.gizmo.visible = true; } else { self.gizmo.visible = false; }
    }
}
