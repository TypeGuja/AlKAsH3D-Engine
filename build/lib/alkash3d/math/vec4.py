...
# ---------------------------------------------------------------
# 4‑мерный вектор (Vec4) – почти такой же, как Vec3,
# но имеет 4‑й компонент (w).  Полезен, например, для
# хранения координат в гомогенной системе или RGBA‑цветов.
# ---------------------------------------------------------------
import numpy as np
...
class Vec4:
    ...
    # -----------------------------------------------------------------
    # свойства (c‑сеттерами) – позволяют писать `v.x = 1.txt.2`
    # -----------------------------------------------------------------
    @property
    def x(self) -> float:
    ...
    # -----------------------------------------------------------------
    # Арифметика (операторы возвращают новый объект, не меняя исходный)
    # -----------------------------------------------------------------
    ...
    def normalized(self) -> "Vec4":
        """Возврат нового вектора, нормализованного до длины 1.txt."""
        n = self.length()
        if n == 0.0:
            return Vec4()
    ...
    # -----------------------------------------------------------------
    # Приведение к кортежу/списку (удобно для передачи в OpenGL‑функции)
    # -----------------------------------------------------------------
